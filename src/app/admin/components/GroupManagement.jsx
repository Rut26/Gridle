...
const GroupManagement = () => {
  ...
  const filterGroups = () => {
    ...
    if (filters.privacy) {
      filtered = filtered.filter(g =>
        filters.privacy === 'public' ? g.isPublic : !g.isPublic
      );
    }
  ...
  const handleDeleteGroup = async (groupId) => {
    if (!window.confirm('Are you sure you want to delete this group? This action will also delete all associated tasks and notes.')) return;
    try {
    ...
    try {
      ...
      if (res.ok) {
        setGroups(prev =>
          prev.map(g =>
            g._id === groupId
              ? { ...g, members: g.members.filter(m => m.user._id !== userId) }
              : g
          )
        );
      }
  ...
  const handleReassignOwnership = async (groupId, newOwnerId) => {
    try {
      ...
      if (res.ok) {
        ...
        setGroups(prev =>
          prev.map(g => g._id === groupId ? { ...g, creator: newOwner } : g)
        );
  ...
  return (
    <div className="bg-card text-card-foreground p-8 rounded-2xl shadow-lg border border-border">
      ...
      {isLoading ? (
        ...
        <>
            ...
            <table className="min-w-full divide-y divide-border">
              <thead>
                <tr className="bg-muted">
                  ...
                  <th className="px-4 py-2 text-left text-sm font-medium text-muted-foreground">Privacy</th>
                  <th className="px-4 py-2 text-left text-sm font-medium text-muted-foreground">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {filteredGroups.length === 0 ? (
                  <tr><td colSpan="5" className="text-center py-4 text-muted-foreground">No groups found.</td></tr>
                ) : (
                  filteredGroups.map(group => (
                    <tr key={group._id}>
                      <td className="px-4 py-3 whitespace-nowrap text-foreground">
                        <button onClick={() => viewGroupDetails(group)} className="text-primary hover:underline">
                          {group.name}
                        </button>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap text-muted-foreground">{group.creator?.name || 'Unknown'}</td>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap space-x-2">
                        <button onClick={() => viewGroupDetails(group)} className="text-primary hover:underline text-sm">View</button>
                        <button onClick={() => { setSelectedGroup(group); setShowMembersModal(true); }} className="text-primary hover:underline text-sm">Members</button>
                        <button onClick={() => { setSelectedGroup(group); setShowJoinCodeModal(true); }} className="text-primary hover:underline text-sm">Join Code</button>
                        <button onClick={() => { setSelectedGroup(group); setShowReassignModal(true); }} className="text-primary hover:underline text-sm">Reassign</button>
                        <button onClick={() => handleDeleteGroup(group._id)} className="text-destructive hover:underline text-sm">Delete</button>
                      </td>
                    </tr>
                  ))
                )}
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            <span className="text-muted-foreground">Page {groupPage} of {groupTotalPages}</span>
            <button
              onClick={() => fetchGroups(groupPage + 1)}
      ...
      {showDetailsModal && selectedGroup && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-card p-6 rounded-lg w-11/12 max-w-4xl max-h-[90vh] overflow-y-auto">
            <h3 className="text-xl font-semibold mb-4">Group Details: {selectedGroup.name}</h3>
            ...
            <div className="mb-4">
              ...
              >
                ...
                {availableUsers.filter(user => 
                  !selectedGroup.members?.some(member => member.user._id === user._id)
                ).map(user => (
      ...
      {showJoinCodeModal && selectedGroup && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-card p-6 rounded-lg w-11/12 max-w-md">
            <h3 className="text-xl font-semibold mb-4">Join Code: {selectedGroup.name}</h3>
      ...
      {showReassignModal && selectedGroup && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-card p-6 rounded-lg w-11/12 max-w-md">
            <h3 className="text-xl font-semibold mb-4">Reassign Ownership: {selectedGroup.name}</h3>
